...
fn init_targets() [TARGET_ROWS * TARGET_COLS]Target {
    ...
    while (row < TARGET_ROWS) : (row += 1) {
        ...
        while (col < TARGET_COLS) : (col += 1) {
            targets[row * TARGET_COLS + col] = Target{ .x = TARGET_GRID_X + (TARGET_WIDTH + TARGET_PADDING_X) * @intToFloat(f32, col), .y = TARGET_GRID_Y + TARGET_PADDING_Y * @intToFloat(f32, row) };
        }
    }
    return targets;
}
